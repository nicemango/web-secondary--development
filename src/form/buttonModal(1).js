...
class ButtonModal extends Component {
  ...
  change = () => {
    ...
    window.location && window.location.search
      ...
      : {};
    const { type, processStatus, setInfo, primaryKey, editInfo, viewId,form } = this.props;
    console.log(type, 'type')
    console.log(processStatus, 'processStatus');
    // let formId = setInfo.id || '';
    let dataId;
    if (search.data_id === 'insert') {
      message.error('请先保存数据');
      return;
    }
    if ((search.data_id === 'edit')) {
      ...
      getContentData({
        ...
        viewId: viewId || '',
        type: form.getFieldValue().protocal_type || "2",
  ...
  callbackDoThing = () => {
    ...
    if (searches.name_en && searches.protocal_id && searches.doc) {
      ...
      //   doc: searches.doc,
      // };
      callBackDoc(searches.name_en, searches.protocal_id, searches.doc).then((res) => {
