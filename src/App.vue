    ...
    <div class="tabPage">
      <div class="statisticalDateOf">
        <div v-show="reportType != '年报'" class="leftBox">
          ...
          <el-button class="marginR" type="primary" size="small" @click="addDate('add')">{{pickerDown}}<i class="el-icon-arrow-right el-icon--right"></i></el-button>
      ...
      <div ref="tableBox" class="tableBox">
        ...
        <div class="tableTop">
          ...
          <div class="content">{{reportObj.machine_volume}}</div>
        </div>
        <div v-if="parmasType != 'object'" class="tableTop">
...
export default {
  ...
  methods: {
    ...
    async getReportsData() {
      ...
      let tableData = await queryReportsData(params).then(res => {
        return res.data;
      })
      this.tableData = tableData.detail.map(x=>{
        // string.replace(/regExp/g, '');
        x.equivalentTime = Number(x.equivalentTime).toFixed(2);
        x.power_output = Number(x.power_output).toFixed(2);
        return x;
      });
      Object.keys(this.reportObj).forEach(x=>{
        this.reportObj[x] = tableData[x]
      })
    },
    // 报表导出
    exportReportsData() {
