<template>
  <div class="monitoring_page">
    <div class="left_box">
      ...
      <div class="list-box">
        <div class="list_title" v-for="item in equipmentfilterData" :key="item.device_id" :class="{'list-active': activeClass == item.device_id}" @click="selectOne(item)">{{item.device_name}}</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async queryAsset() {
      ...
      this.equipmentData = await queryAssetById(dataParams,params).then(res=>{
        let key = res.data[0]
      ...
      this.selectOne(this.equipmentData[0])
      console.log('this.equipmentData',this.equipmentData);
    },
    // 直播请求
    async queryLiveing() {
      await queryLiveing(this.activeClass).then(res=>{
    ...
    async alarmAsset() {
      ...
      this.alarmData = await queryAssetById(dataParams, params).then(res=>{
        let key = res.data[0]
        let value = res.data[1]
        let data = value.map(val => {
          let obj = {};
    ...
    async queryimg() {
      ...
      await queryWarnPicture(params).then(res=>{
