...
function setupNormalizeAndQuantize() {
	...
	if (playerElement && videoElement.length > 0) {
		...
		if (playerAspectRatio > videoAspectRatio) {
			...
			let unquantizeAndDenormalizeUnsigned = (x, y) => {
...
function connect() {
	...
	// ws = new WebSocket('ws://10.15.111.24:90/')
	ws = new WebSocket(wsUrl) // wsUrl
...
function setBandwidthCap(cap) {
  let capBpsCmd = 'Encoder.MaxBitrate ' + cap;
  let descriptor = {
      Console: capBpsCmd
  }
  // let capStr = cap != 0 ? cap : 'Unlimited';
